...
func LoadV1OrderCommunicationAPI(v *viper.Viper, router *http.ServeMux, gormdb *gorm.DB, storage_client *storage.Client, twilio_client *twilio.RestClient, forms_service *forms.Service, orderchan chan int) {
	...
	router.HandleFunc("/v1/order_communication/list", ListOrders(v, gormdb))
	router.HandleFunc("/v1/order_communication/deliver_to_kitchen", DeliverOrderToKitchen(v, gormdb, twilio_client, orderchan))
...
func ListOrders(v *viper.Viper, db *gorm.DB) http.HandlerFunc {
...
func DeliverOrderToKitchen(v *viper.Viper, db *gorm.DB, twilioc *twilio.RestClient, orderchan chan<- int) http.HandlerFunc {
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		cors(v, w, r)
